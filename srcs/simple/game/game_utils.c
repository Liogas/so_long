...
/*   Updated: 2024/03/17 22:01:10 by glions           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	load_img_exit_player(t_game *game)
{
	...
	if (!floor)
		return (0);
	exit = image42_file_new(64, 64, "./srcs/simple/imgs/exit.xpm", game->mlx);
	if (!exit)
		return (image42_free(floor), 0);
	player = image42_file_new(64, 64, "./srcs/simple/imgs/w_down.xpm", game->mlx);
	if (!player)
...
int	load_img_exit(t_game *game)
{
	...
	if (!floor)
		return (0);
	img = image42_file_new(84, 64, "./srcs/simple/imgs/exit.xpm", game->mlx);
	if (!img)
		return (0);
...
t_image42	*load_img_object(t_game *game, int id)
{
	...
	if (!floor)
		return (printf("fjeijeijfe\n"), NULL);
	if (id == 0)
		object = image42_file_new(64, 64, "./srcs/simple/imgs/s_1.xpm", game->mlx);
	else if (id == 1)
		object = image42_file_new(64, 64, "./srcs/simple/imgs/s_2.xpm", game->mlx);
	else if (id == 2)
		object = image42_file_new(64, 64, "./srcs/simple/imgs/s_3.xpm", game->mlx);
	if (!object)
		return (printf("Errreur ehufhwegihug\n"), image42_free(floor), NULL);
