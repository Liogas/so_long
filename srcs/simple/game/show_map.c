...
/*   Updated: 2024/03/14 14:52:19 by glions           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_img(int value, t_game *game, int i, int j)
{
	...
	if (value == 1)
	{
		img = image42_file_new(64, 64, "./srcs/simple/imgs/water.xpm",
				game->mlx);
		if (!img)
			return (printf("Erreur chargement image water\n"), 0);
	}
	else
	{
		img = image42_file_new(64, 64, "./srcs/simple/imgs/floor.xpm",
				game->mlx);
		if (!img)
